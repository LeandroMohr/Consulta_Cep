...
namespace AppCep
{
    ...
    public class RestCepController : Controller
    {
        ...
        public IEnumerable<Endereco> BuscarCep(string cep)
        {
            var endereco = _context.Enderecos.Where(s => s.Cep.Replace("-","") == cep);
            return endereco;
        }

        [HttpGet("Enderecos/EnderecosPorEstado/{uf}")]
        public IEnumerable<Endereco> BuscarEstado(string uf)
        {
